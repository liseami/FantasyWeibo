...
struct TweetMediaView: View {
    ...
    init(urls : [String],cliped : Bool = true ) {
        self.urls = urls
        self.cliped = cliped
        if urls.count == 1 {
...
struct PostPicsView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        TweetMediaView(urls: pic1)
            .previewLayout(.fixed(width: SW,height: SW * 2))
        TweetMediaView(urls: pic2)
            .previewLayout(.fixed(width: SW,height: SW * 2))
        TweetMediaView(urls: pic3)
            .previewLayout(.fixed(width: SW,height: SW * 2))
        TweetMediaView(urls: pic4)
            .previewLayout(.fixed(width: SW,height: SW * 2))
        TweetMediaView(urls: pic5)
            .previewLayout(.fixed(width: SW,height: SW * 2))
        TweetMediaView(urls: pic6)
            .previewLayout(.fixed(width: SW,height: SW * 2))
        TweetMediaView(urls: pic7)
            .previewLayout(.fixed(width: SW,height: SW * 2))
        TweetMediaView(urls: pic8)
            .previewLayout(.fixed(width: SW,height: SW * 2))
        TweetMediaView(urls: pic9)
            .previewLayout(.fixed(width: SW,height: SW * 2))
